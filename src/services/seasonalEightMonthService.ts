...
export function generateEightMonthSeasonalTrends(
  ...
  // 24 decades of 10 days = 240 days (8 full calendar months)
  for (let dNum = 1; dNum <= 24; dNum++) {
    const decadeInMonth = ((dNum - 1) % 3) + 1;
    const startDayOffset = (dNum - 1) * 10 + 1;
    const endDayOffset = dNum * 10;
    ...
    // Scale-adapted monthly normals (1991-2020)
    ...
    const decadeLabel = decadeInMonth === 1 
      ? `1ère Décade (1er au 10 ${monthName})` 
      : decadeInMonth === 2 
        ? `2ème Décade (11 au 20 ${monthName})` 
        : `3ème Décade (21 à fin ${monthName})`;

    const dateRangeFormatted = `${dStart.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} - ${dEnd.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}`;

    const isWinterDecade = mIdx === 11 || mIdx === 0 || mIdx === 1 || mIdx === 2;
    const isAutumnDecade = mIdx >= 8 && mIdx <= 10;
    const isSpringDecade = mIdx >= 3 && mIdx <= 4;
    const isSummerDecade = mIdx >= 5 && mIdx <= 7;
  ...
  // ==========================================
  // AGGREGATE 24 DECADES INTO 8 CLEAR MONTHS
  // ==========================================
  const months: MonthlySeasonalProjection[] = [];
  for (let m = 0; m < 8; m++) {
    const monthDecades = decades.slice(m * 3, m * 3 + 3);
    const firstDecade = monthDecades[0];
    const mIdx = firstDecade.monthIndex;
    const monthName = firstDecade.monthName;

    // Approximate Year computation based on starting month
    const startYear = now.getFullYear();
    const currentMonthIdx = now.getMonth();
    const year = mIdx < currentMonthIdx ? startYear + 1 : startYear;
    ...
    months.push({
      ...
      dominantSynopticRegime: firstDecade.dominantSynopticRegime,
      synopticRegimeCode: firstDecade.synopticRegimeCode,
