  ...
  if (hasRealThunderstorms) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (sDist <= 25) {
        ...
        stormNarrative = `Amas convectif orageux N°${i + 1} situé à moyenne portée à ${sDist} km au secteur ${sCompass}. La signature radar Doppler présente une réflectivité de ${sDbz} dBZ sur des têtes de cumulonimbus grimpant à FL${sFl} (${sAltKm} km). L'activité foudre reste modérée avec ${strikes15} impacts/15min. Ce foyer convectif demeure en surveillance renforcée par nos calculs de trajectoire Doppler.`;
      } else {
